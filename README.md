...
- ✦ AI 幫我：排場佈/排地墊/畫動線/檢查/改善/夥伴圖 — 一律先預覽再套用。
  離線規則／關鍵字，不是雲端模型；沒有金鑰也能用
- 檢查中心：超界/重疊/擋門/走道過窄/視線 等規則（門檻在進階可調）
- 三段 Responsive Workspace（手機 ≤600 / 平板 601–1199 / 桌機 ≥1200），
  Canvas-first、單列 Header、Bottom Sheet
- 本機自動儲存（含損壞自動備援）、具名平面圖、JSON 匯入匯出
- PWA：本機／preview 可離線；有新版本時提示更新、「更多 → 關於」可看版本/commit/建置時間，
  部署後 `/version.json` 可查線上版本。真機安裝／離線尚未在 production 過關
- 已知限制：更多 → 已知限制，或 [`docs/PRODUCT_LIMITATIONS.md`](docs/PRODUCT_LIMITATIONS.md)
  （掃描場地是假資料、方案版本仍是全域 named layout、兩個分頁後寫贏等）
