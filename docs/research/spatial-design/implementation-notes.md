...
- 《建築物無障礙設施設計規範》：通路走廊淨寬不得小於 120 公分；
  門扇開啟後仍須維持該淨寬。另有通路淨寬 130 公分的條文。
- 輪椅迴轉空間通常以直徑 150 公分判斷，**但研究過程沒有取得該條原文**。
- NFPA 101 的 egress width 是**依容留人數計算**，不是單一數字；
  **條文原文未取得**。
- ADA accessible route 36 吋、迴轉空間 60 吋，**條文原文未取得**。
...
- 知識條目 `accessibility-corridor-width`（medium）、
  `accessibility-turning-space`（**low**，limitations 寫明「未取得該條完整條文原文」）、
  `egress-not-a-single-number`（medium，寫明本工具不做此計算）、
  `door-clearance`（medium，寫明 1.2 公尺是本工具的預設設計值而非法規要求）。
- 四條全部 `requiresHumanReview: true`。
