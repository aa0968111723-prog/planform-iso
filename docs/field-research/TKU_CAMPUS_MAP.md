...
| E310 | 54 | `venue:tku-e310`（既有場地模板，維持原資料） |
| E305 | 照片參考 | `venue:tku-e305`（門牌與窗型冷氣標記為 E305。尺寸待現場校正。**不是 E310**） |
...
1. 新建專案 → E310 實景或 E305 照片參考場地（兩者分開，照片不會互蓋）
2. 場地分頁「淡江校園位置」或新生夥伴視圖開啟 **OpenStreetMap＋Leaflet** 地圖（不需 API Key）
3. 流程：校園 → 樓館 → 樓層 → 教室 → 入口待現場確認 → 進入室內場佈
4. 樓館公開座標只標「樓館位置」，不捏造室內房間 GPS
5. 圖資失敗或離線時改顯示已儲存的校園／樓館／教室目錄，不會白屏
6. `✦ AI 幫我` 能讀教室代碼與地名；未建檔代碼落到該棟通用起點
