...
1. 新建專案 → E310 實景／E305 照片參考／淡江常用場地＋「🧭 新生夥伴視圖」
2. 新生視圖的校園位置圖：OpenStreetMap＋Leaflet（`src/ui/campusMap.ts`），
   三個實體校園可切換；搜尋樓館代碼、名稱或教室代碼（E305、E310、SG320）
3. 目前活動場地用明顯標記，其他樓館降低權重；標籤重疊時只留較重要的
4. 地圖失敗或離線：顯示已儲存的校園／樓館／教室目錄，不是白屏
5. 「進入室內場佈」打開俯視場佈：入口、報到、鞋子、背包、青綠色地墊、講師
6. `✦ AI 幫我` 能讀教室代碼與地名
7. 輸入未建檔代碼會落到該棟通用起點

**E305 與 E310 分開。** E305 是門牌／冷氣標記清楚的照片參考場地，尺寸待現場校正；
不得把 E305 照片套到 E310。尚未確認門牌的照片標「照片場地尚未綁定」。
