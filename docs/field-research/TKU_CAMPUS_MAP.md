...
| 場地 | 次數 | 入口 |
|---|---|---|
| E308 | 101 | `venue:tku-classroom` |
| E305 | — | `venue:tku-e305` 照片參考，不是 E310 |
| SG109 社辦 | 100 | `venue:tku-classroom` |
...
1. 新建專案 → E310 實景、E305 照片參考、淡江常用場地
2. 「🗺️ 新生」打開校園位置圖：校園 → 樓館 → 樓層 → 教室 → 入口 → 室內場佈
3. 地圖是 OpenStreetMap＋Leaflet，離線時改顯示已儲存的樓館／教室目錄
4. 樓館公開座標只標「樓館位置」。教室入口未實測就寫「入口待現場確認」
5. `✦ AI 幫我` 能讀教室代碼與地名
6. 輸入未建檔代碼會落到該棟通用起點

尺寸要成真：現場量地磚、門寬、已知牆距。
E305 與 E310 是兩間教室，照片不得互套。
