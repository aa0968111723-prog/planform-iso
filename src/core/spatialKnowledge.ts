...
export const KNOWLEDGE_BASE: readonly KnowledgeEntry[] = [
  ...
  /* ---------------- accessibility / safety ---------------- */
  {
    ...
    summary:
      "《建築物無障礙設施設計規範》要求通路走廊寬度不得小於 120 公分，且走廊中有門時，" +
      "扣除門扇開啟空間後淨寬仍不得小於 120 公分。另有通路淨寬不得小於 130 公分的條文，" +
      "獨棟或連棟建築物得為 90 公分以上。",
    rules: [
      { statement: "無障礙通路走廊淨寬下限", value: 1.2, unit: "m", appliesTo: "checkAccessibilityWarnings" },
      { statement: "門扇開啟後仍須維持的淨寬", value: 1.2, unit: "m", appliesTo: "checkDoorClearance" },
    ],
    examples: ["排走道時，把最小走道寬度設成 1.2 公尺，可以先排除明顯過窄的配置。"],
    confidence: "medium",
  ...
  },
  {
    ...
    category: "accessibility",
    summary:
      "台灣無障礙規範通常以直徑 150 公分的迴轉空間作為輪椅轉向的基本判斷；" +
      "ADA Standards 則要求 60 英吋（約 1525 公釐）直徑的迴轉空間。兩套法規體系需分別判讀，不可互相取代。",
    rules: [
      { statement: "輪椅迴轉空間直徑（台灣慣用判斷值）", value: 1.5, unit: "m", appliesTo: "checkAccessibilityWarnings" },
    ],
    examples: ["報到桌前若要能讓輪椅轉向，桌前應保留至少直徑 1.5 公尺的淨空。"],
    confidence: "low",
    sourceUrl: "https://law.moj.gov.tw/LawClass/LawAll.aspx?pcode=D0070115",
    sourceType: "government",
    retrievedAt: R,
    applicableVenueTypes: ["any"],
    limitations: [
      "研究過程未取得該條完整條文原文，數值需以官方 PDF 逐條核對後才可引用。",
      "ADA 的 60 英吋數值同樣需以 ada.gov 官方標準核對。",
  ...
  },
  {
    ...
    summary:
      "NFPA 101 的避難寬度依容留人數、構件類型與用途計算，不是固定值；" +
      "台灣的避難層出入口規定也與建築用途、樓梯總寬、門扇開啟方向一併判定。" +
      "任何把避難寬度化約成單一數字的說法都應被視為不完整。",
    rules: [
      { statement: "避難寬度需依容留人數與用途計算，本工具不做此計算。" },
    ],
    ...
    applicableVenueTypes: ["any"],
    limitations: [
      "研究過程未取得 NFPA 101 條文原文，僅確認其為情境式計算。",
